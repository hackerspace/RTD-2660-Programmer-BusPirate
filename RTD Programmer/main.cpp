...
const char* USAGE = "Usage options:\n"
	...
	"Options may be combined, i.e. -r backup.bin -w newfw.bin will first backup the dump, and then flash new one.\n";

bool bSave = false, bWrite = false, bErase = false;
const char * sInfile = nullptr, * sOutfile = nullptr;
...
{
	E_CC_NOOP = 0, E_CC_WRITE = 1, E_CC_READ = 2, E_CC_WRITE_AFTER_WREN = 3, E_CC_WRITE_AFTER_EWSR = 4, E_CC_ERASE = 5
};
...
uint32_t
SPICommonCommand ( ECommondCommandType cmd_type, uint8_t cmd_code, uint8_t num_reads, uint8_t num_writes
							, uint32_t write_value )
{
	num_reads &= 3;
	num_writes &= 3;
	write_value &= 0xFFFFFF;
	uint8_t reg_value = (cmd_type << 5) | (num_writes << 3) | (num_reads << 1);

	WriteReg ( 0x60, reg_value );
	WriteReg ( 0x61, cmd_code );
	switch ( num_writes )
	{
	case 3: WriteReg ( 0x64, write_value >> 16 );
		WriteReg ( 0x65, write_value >> 8 );
		WriteReg ( 0x66, write_value );
		break;
	case 2: WriteReg ( 0x64, write_value >> 8 );
		WriteReg ( 0x65, write_value );
		break;
	case 1: WriteReg ( 0x64, write_value );
		break;
	}
...
void SPIRead ( uint32_t address, uint8_t * data, int32_t len )
{
	...
	while ( len>0 )
	{
		...
		if ( read_len > MAX_READLEN )
			read_len = MAX_READLEN;
...
const char * GetManufacturerName ( uint32_t id )
{
	switch ( id )
	{
	case 0x20: return "ST";
...
{
	printf ( "%s", GetManufacturerName ( id ) );
}
...
void SetupChipCommands ( uint32_t jedec_id )
{
	...
	{
	case 0xEF:
		printf ("Setup chip commands for Winbond...");
		...
		printf ("done\n");
		break;
	default: printf ( "Can not handle manufacturer code %02x\n", manufacturer_id );
		exit ( -6 );
		break;
	}
}

static bool ShouldProgramPage ( uint8_t * buffer, size_t size=256 )
{
...
void TestAndDump ( int fd, uint8_t * buf, uint32_t * pskip )
{
	const int PGSIZE=256;
	if ( !ShouldProgramPage (buf) )
		++(*pskip);
	else
	{
		uint8_t bufff[PGSIZE] = {0xff};
		for ( int i=0; i<*pskip; ++i)
			::write (fd, bufff, PGSIZE);
		::write(fd,buf,PGSIZE);
		*pskip = 0;
	}
}
...
{
	int fdump = ::open ( output_file_name, O_CREAT | O_RDWR | O_TRUNC, 0644 );
	...
	do
	{
		...
		TestAndDump ( fdump, buffer, &skip );
		TestAndDump ( fdump, buffer + 256, &skip );
		TestAndDump ( fdump, buffer + 512, &skip );
		TestAndDump ( fdump, buffer + 768, &skip );
	...
	printf ( "Chip CRC %02x\n", chip_crc );
	printf ( "Dumped %dKB\n", (chip_size-skip*256)/1024);
	return data_crc==chip_crc;
}

int64_t GetFileSize ( int iFD )
{
	if ( iFD<0 )
	{
		printf ( "invalid descriptor to fstat '%d'", iFD );
		return -1;
	}
...
static uint8_t * ReadFile ( const char * file_name, uint32_t * size )
{
	...
	int fdfile = ::open ( file_name, O_RDONLY, 0644 );
	uint8_t * result = NULL;
	if ( fdfile==-1 )
	{
	...
	int64_t file_size64 = GetFileSize ( fdfile );
	...
	if ( file_size64<0 ) // error message is already printed by GetFileSize
		return result;

	uint32_t file_size = (uint32_t) file_size64;
	result = new uint8_t[file_size];

	::read ( fdfile, result, file_size );
	//fread ( result, 1, file_size, file );
	::close ( fdfile );
	//fclose ( file );
	if ( !memcmp ( "GMI GFF V1.0", result, 12 ) )
	{
		...
		uint32_t gff_size = ComputeGffDecodedSize ( result + 256, file_size - 256 );
		if ( gff_size==0 )
		{
...
bool ProgramFlash ( const char * input_file_name, uint32_t chip_size )
{
	...
	printf ( "Will write %dKb\n", prog_size / 1024);

	if (!bErase) // if it is set, chip is already explicitly erased
		EraseFlash ();
...
int main ( int argc, char * argv[] )
{
	...
	if ( !InitI2C () )
	{
		printf ( "Can't connect to the I2C device. Check the cable.\n" );
		return -1;
	}
	printf ( "Ready\n" );
	SetI2CAddr ( 0x4a );
	...
	for (;;)
	{
		...
		static struct option long_options[] = {
			{"help", no_argument, 0, '?'},
			{"save", required_argument, 0, 'r'},
			{"flash", required_argument, 0, 'w'},
			{"erase", no_argument, 0, 'e'},
			{ 0, 0, 0, 0},
		};
		int c = getopt_long(argc, argv, "r:w:e?", long_options, &option_index);
		if (c==-1)
			break;
		switch (c) {
		case 'r':
	...
	if ( bSave )
	{
		printf ("Save dump to '%s'\n", sOutfile);
		SaveFlash (sOutfile, chip->size_kb*1024);
	}

	if ( bErase )
	{
		printf ( "will erase the chip\n" );
		EraseFlash ();
	}
	if ( bWrite )
	{
		ProgramFlash ( sInfile, chip->size_kb * 1024 );
	}
